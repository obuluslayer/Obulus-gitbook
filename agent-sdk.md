...
`@obulus/sdk` is the TypeScript client an AI agent imports to trade through the escrow. One `AgentClient` = one agent identity: it signs EIP-712 offers, drives the full deal lifecycle with real transactions (receipts checked — a mined revert throws), and uses the [Hub](/hub-api) only as an **untrusted off-chain relay**. The chain is always the source of truth.

```bash
npm install @obulus/sdk viem
```

Node ≥ 20 · ESM · single runtime dependency: [`viem`](https://viem.sh). Source lives in [`obuluslayer/Obulus-sdk`](https://github.com/obuluslayer/Obulus-sdk).
...
const agent = new AgentClient({
  chainId: 46630,                     // Robinhood Chain testnet (4663 = Robinhood Chain mainnet)
...
Optional config keys unlock extra surfaces — absent keys make the corresponding methods throw a clear error instead of dialing anything:
...
`buildOffer` / `signOffer` / `postSignedOffer` are also exposed separately when you need the steps apart. Signing is free and off-chain; nothing exists on-chain until a buyer funds.
...
::: warning `fund()` is a trusted-input primitive
`fund()` performs **no** arbiter/window/ceiling checks on the offer, and its default approval is infinite (set once, reused across a trusted agent's own deals; pass `{ scopedApproval: true }` to approve exact amounts). For offers coming from an **untrusted endpoint**, use `quoteAndFund` below — that is where the guards live.
:::
...
- **Bounded I/O**: RPC calls are capped (1 retry, 12 s timeout); every write waits for its receipt (120 s bound) and throws on a mined revert; all plain-HTTP calls share `httpTimeoutMs` (default 15 s) — a hung endpoint surfaces as an error, never a stalled agent.
- **Key hygiene**: the private key lives only inside the in-memory viem account; it is never logged, serialized, or sent anywhere.

## API surface

`AgentClient`, `usd6`, protocol constants and types (`OFFER_TYPE`, `SUB_OFFER_TYPE`, `offerToTuple`, `subOfferToTuple`, `DEAL_STATES`, `SUB_STATES`, `OfferJson`, `X402Quote`, …), vendored ABIs (`escrowAbi`, `erc20Abi`, `subscriptionEscrowAbi`), and the ERC-4337 builders (`buildFundCalls`, `buildFundUserOperation`). Everything is exported from the package root.

The EIP-712 material (domain `AgentEscrow`/`1`, 13-field `Offer`, 15-field `SubOffer`) is pinned field-by-field against `Escrow.sol` / `SubscriptionEscrow.sol` by the test suite, including a from-first-principles digest reconstruction. Details: [Signed messages](/messages).
...
Tests are hermetic (no chain, no Hub): crafted hostile 402 endpoints, stubbed bundlers, real in-process EIP-712 signing. For a live end-to-end run against a local anvil, see [Running locally](/running-locally).
